...
const RECOVERY_KEY = 'health.sw-recovery-attempted';

/**
 * Self-update — the Angular wiring. The rules live in
 * `@xinutec/ui-harness/sw-updates`; this is the adapter.
 *
 * This dashboard is read on a phone, so the shell is cached and the app opens
 * without waiting for the network. ⚠ **ngsw alone would then cache a build that
 * never learns a newer one exists** — which is worse than no caching, because it
 * looks fine. That is why the update path arrives in the same change
 * (dev-lint#1384).
 *
 * ⚠ **Only the SHELL is cached here — `ngsw-config.json` carries no dataGroups.**
 * Every other adopter caches its API with a freshness strategy; this one must not.
 * A reading is a measurement with a time on it, and a cached body weight or sleep
 * score rendered as the current one is a wrong answer rather than a slow one. So
 * offline this app opens and shows nothing, which is the honest state.
 *
