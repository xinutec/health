...
export interface EpisodeGeometry {
  ...
  endTs: number;
  mode: DayState["mode"];
  // Mirrors the backend EpisodeKind union (Verified/Geo/EpisodeGeometry.lean).
  // Still written by hand — the frontend has no compile-time link to the
  // backend types — but no longer only by hand: rust/backend/tests/
  // frontend_unions.rs fails the gate when this list and the backend's
  // disagree (#337).
