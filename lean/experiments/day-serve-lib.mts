 ...
 * loop (#431 gap 3). Two copies would have been two copies of the two things
 * here that are subtle and were both wrong once: the miss regex has to anchor on
		...
		for (const m of misses) {
			...
			switch (m.what) {
				...
				case "nearbyStations": {
					...
					partial.nearbyStations[traceKey(lat, lon, rad)] = await orNothing(() => osm.nearbyStations(lat, lon, rad), []);
					break;
				}
