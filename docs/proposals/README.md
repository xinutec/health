...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V1 complete; V2 cron shadow live in prod; V3 shortest path proved | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM decoding fully theorem-backed down to the packed production-speed decoder (`pDecode`, 4.4s/87MB per day); `npm run lean-check` (in `verify`) gates on `#guard`s + the parity harness; and **the decode-recent cron A/Bs every decoded day against the verified decoder in production — exact on every real day so far (24/24 corpus replays incl. C4 flags, 7/7 live soak), quantisation lossless**; grep pod logs for `lean-shadow`. V3 (rail-snap): the shortest path is **proved via a certifying checker** (`dijkstraC_correct` — returned paths attain the oracle minimum; certification failure → honest `none`), and `npm run compare-rail` pins TS↔Lean path identity on real corridors. Next: rail snap-contract layer → map-match-core → shell |
