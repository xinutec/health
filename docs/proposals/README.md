...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V1 complete; V2 shadow live | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM decoding fully theorem-backed (verified decoder `decode_correct`/`decode_none_iff`, memoised to production asymptotics); `verified_cli` runs it; `npm run lean-check` (in `verify`) gates on `#guard`s + the parity harness; and **`npm run lean-shadow` A/Bs the verified decoder on the real golden corpus — 12/12 days exact, quantisation lossless**. Next: shadow in the decode-recent cron → rail-snap → map-match-core → shell |
