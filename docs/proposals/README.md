...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V0 + V1 spec layer shipped | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM trellis pilot with spec, brute-force oracle (sound + complete, proved), the backward Bellman recurrence and the forward Viterbi-principle DP both proved equal to the oracle, build-time `#guard` parity, and a TS↔Lean parity harness green at day scale. Remaining phases: V1 array refinement (trellis columns ≡ `bestEnd`, backtrack) → real-data shadow → rail-snap → map-match-core → shell |
