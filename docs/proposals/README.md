...
| `2026-07-venue-measurement-model.md` | design (no code) | **Venue attribution as a calibrated measurement model.** Stop scoring venues on distance; score them on likelihood — a venue is an EXTENT, and the sensor's lie is LEARNED (fit the residual on unambiguous stays, never on the model's own hard-case guesses). Fixes the wall-vs-pin incommensurability at the root and the near-field short-circuit that hides it. Reference case: Urban Social named after the pub next door (#325). Phases V0–V4 |
