...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V0 shipped | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM trellis pilot with spec, brute-force oracle, build-time `#guard` parity, `enum_sound` proved, and a TS↔Lean parity harness green at day scale. Phases V1–V5 (equivalence theorem → real-data shadow → rail-snap → map-match-core → shell) |
