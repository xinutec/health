...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V1 COMPLETE | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM decoding fully theorem-backed — oracle sound+complete, backward Bellman, forward Viterbi-principle DP, the trellis column recurrence, and a verified decoder (`decode_correct`/`decode_none_iff`), memoised to production asymptotics with theorems inherited (`decodeFast_eq`); `verified_cli` runs it, and the TS↔Lean harness agrees exactly at day scale. Next: Score genericisation (nicety) → V2 real-data shadow → rail-snap → map-match-core → shell |
