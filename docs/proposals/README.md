...
| File | Status | Topic |
|---|---|---|
| `2026-07-verified-core-lean.md` | V1 proved (spec level) | **Port decision logic to a total, theorem-proving language (Lean 4), toward a proved core + thin effectful shell.** The `lean/` package: HSMM decoding fully theorem-backed — oracle sound+complete, backward Bellman, forward Viterbi-principle DP, the trellis column recurrence, and a verified decoder (`decode_correct`: any returned path renders a well-formed segmentation achieving the oracle best; `decode_none_iff`), `#guard`-pinned against the imperative trellis paths-and-all, TS↔Lean harness green at day scale. Remaining: memoise the proved decoder (mechanical) → real-data shadow → rail-snap → map-match-core → shell |
