...
async function loadRecoveryRows(userId: string, since: string): Promise<RecoveryRows> {
	...
	const sleepRows = await db()
		...
		.execute();
	return {
		hrv: hrvRows.map((r) => ({ date: ymd(r.date), value: r.daily_rmssd == null ? null : Number(r.daily_rmssd) })),
		rhr: rhrRows.map((r) => ({ date: ymd(r.date), value: r.resting_heart_rate ?? null })),
		sleep: sleepRows
			.filter((r) => r.is_main_sleep)
			.map((r) => ({ date: ymd(r.date), value: r.minutes_asleep == null ? null : Number(r.minutes_asleep) / 60 })),
	};
}
