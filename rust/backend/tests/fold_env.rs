...
//! `fixtures/fold-env-ts.mjs`; every coordinate is shifted — including the
//! trace KEYS and their bit-encoded twins, folded consistently on both sides
//! (#859) — so the fixture is committable while the corpus stays gitignored.
//!
...
fn every_day_encodes_its_observations_as_the_typescript_does() {
    ...
    for day in &days {
        ...
        for (k, expected) in want {
            ...
            if k == "lookups" {
                // Compared per SECTION and per ROW, because one huge
                // assert_eq over the whole lookups object names nothing: the
                // #859 fixture fold was debugged entirely from this readout.
                let got_lk =
                    encode_lookups(cap.get("trace"), cap.get("tzAt"), cap.get("bestPlace"))
                        .expect("lookups encode");
                let (go, eo) = (
                    got_lk.as_object().expect("obj"),
                    expected.as_object().expect("obj"),
                );
                for (sec, ev) in eo {
                    let gv = go.get(sec).expect("section");
                    if gv != ev {
                        let (ga, ea) = (gv.as_array().unwrap(), ev.as_array().unwrap());
                        eprintln!(
                            "{name}: section {sec}: got {} rows, want {}",
                            ga.len(),
                            ea.len()
                        );
                        for (i, (g, e)) in ga.iter().zip(ea.iter()).enumerate() {
                            if g != e {
                                eprintln!(
                                    "  row {i}:
    got  {}
    want {}",
                        ...
                        panic!("{name}: env.lookups.{sec} differs");
                    }
                }
                continue;
            }
